...
:: 32 and 64 bit Uninstallstrings
...
IF "%verifyUpgrade%"=="Yes" (GOTO :verifyVersion)
...
FOR /f "tokens=2*" %%A in (
  ...
  FOR /f "delims=" %%I in ('reg query "%x64GUID%" /s /f "%%B" 2^>nul ^| FIND "HKEY_LOCAL_MACHINE"') do (
    FOR /f "tokens=2*" %%M in (
      'reg query "%%I" /v "UninstallString" 2^>nul ^| FIND "UninstallString" ^| FIND "MsiExec.exe"'
    ) do (
      SET uninstallStr=%%N
...
FOR /f "tokens=2*" %%A in (
  ...
  FOR /f "delims=" %%I in ('reg query "%x86GUID%" /s /f "%%B" 2^>nul ^| FIND "HKEY_LOCAL_MACHINE"') do (
    FOR /f "tokens=2*" %%M in (
      'reg query "%%I" /v "UninstallString" 2^>nul ^| FIND "UninstallString" ^| FIND "MsiExec.exe"'
    ) do (
      SET uninstallStr=%%N
